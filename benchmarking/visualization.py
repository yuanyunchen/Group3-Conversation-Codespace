...
def create_suite_focus_chart(
    ...
    *,
    ...
    focus_type_name: Optional[str] = None
    ...
    if numeric_axis and isinstance(list(type_series.values())[0][0][0], (int, float)):
        for tname in selected_types:
            ...
            for x_val, y_val, label_text in zip(x_values, y_values, labels):
                axis.annotate(
                    f"{y_val:.4f}\n{label_text}",
                    xy=(x_val, y_val),
                    xytext=(0, 6),
        ...
        for idx, tname in enumerate(selected_types):
            ...
            for category in categories:
                match = next((score for x_val, score, label in series_points if label == category), 0.0)
                scores.append(match)
            offsets = [pos + (idx - (len(selected_types) - 1) / 2) * bar_width for pos in bar_positions]
            axis.bar(offsets, scores, width=bar_width, label=tname)
            for x_pos, score in zip(offsets, scores):
                axis.annotate(
                    f"{score:.4f}",
                    xy=(x_pos, score),
                    xytext=(0, 3),
