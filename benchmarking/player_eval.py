...
def _allocate_multi_counts(total_players: int, ratio: float, target_count: int) -> tuple[list[int], int]:
...
def _single_grid_suite(config: SinglePlayerConfig, seeds: Iterator[int]) -> BenchmarkSuite | None:
    ...
    if not (config.grid_lengths and config.grid_memory_sizes and config.grid_ratio_steps and config.grid_player_counts):
        return None
