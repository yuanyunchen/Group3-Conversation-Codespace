...
class ZipperPlayer(Player):
	def __init__(self, snapshot: PlayerSnapshot, ctx: GameContext) -> None:  # noqa: F821
		...
		optimal_p = ZipperPlayer.best_p_value(self)
		...
		for item in self.memory_bank:
			...
			if item.importance > optimal_p-.01:
				count+=1
	...
	def best_p_value(self):
		...
		return min(1 - at_least_4, 1 - optimal-.07)
	...
	def propose_item(self, history: list[Item]) -> Item | None:
		...
		if (
			...
			ZipperPlayer.readd(self, self.ID_dict[self.previous_item[0]])
